...
const ShapeFileContainer = ({
    ...
    CoucheDonneeKeyToShow = [
        { oldName: "NAME_1", newName: "Region" },
        { oldName: "nom_zone", newName: "Nom zone" },
        { oldName: "description", newName: "Description" },
        { oldName: "superficie", newName: "Superficie (Ha)" },
    ],
    showName = true,
    showPopUp = false
}) => {
        ...
        try {
            ...
            const geoJsonLayer = L.geoJSON(geojson, {
                ...
                onEachFeature: (feature, layer) => {
                    ...
                    // Le code du popup a été commenté, mais il est laissé ici pour référence
                    let popupContent = "";
                    if (CoucheDonneeKeyToShow === "*") {
                        // ⚡ Mode sans filtre : toutes les clés/propriétés sont affichées telles quelles
                        popupContent = Object.keys({ ...feature.properties, ...coucheObject })
                            .map((key) =>
                                CoucheDonneePopupContent(
                                    key, // on garde le nom original
                                    coucheObject[key] || feature.properties[key] || "--"
                                )
                            ).join("");
                    } else {
                        // ⚡ Mode filtré : on ne garde que les clés définies dans CoucheDonneeKeyToShow
                        popupContent = Object.keys({ ...feature.properties, ...coucheObject })
                            .map((key) => {
                                const currentElement = CoucheDonneeKeyToShow.find(
                                    ({ oldName }) => oldName.toLowerCase() === key.toLowerCase()
                                );

                                return currentElement
                                    ? CoucheDonneePopupContent(
                                        currentElement.newName,
                                        coucheObject[key] || feature.properties[key] || "--"
                                    )
                                    : "";
                            }).join("");
                    ...
                    // Bind popup to layer for normal click behaviour BUT also create independent popup instances
                    if (popupContent) {
                        // bindPopup avec options pour permettre plusieurs popups ouvertes
                        try {
                            layer.bindPopup(popupContent, { autoClose: false, closeOnClick: false });
                        } catch (e) {
                            // bindPopup peut échouer sur certains objets ; on laisse tomber silencieusement
                            console.warn("bindPopup failed:", e);
                        }

                        // Crée une instance de popup positionnée au centre (pour showPopUp global)
                        if (center) {
                            const popup = L.popup({ autoClose: false, closeOnClick: false })
                                .setLatLng(center)
                                .setContent(popupContent);
                            addPopupInstance(coucheObject.filePath, popup);

                            // si showPopUp est true lors du chargement, on l'ouvre tout de suite
                            if (showPopUp) popup.openOn(map);
                        }
                    }
